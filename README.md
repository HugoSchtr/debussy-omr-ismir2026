...
|---|---|---|
| **Table 4** | SOTA OMR models on their own training-distribution test sets (GrandStaff, Camera-GrandStaff, OLiMPiC) | `zero_shot_eval/smt/run_smt_evaluation_optimized.py --config config.yaml` (system SMT); `zero_shot_eval/smt/run_smt_evaluation_fullpage.py --config config_fullpage.yaml` (full-page SMT); `zero_shot_eval/zeus/eval_zeus.py --model <checkpoint>` (Zeus, 3 checkpoints); `e2e_baselines/eval_e2e.py --checkpoint <ckpt> --arch {FCN,CRNN,CNNT}` |
...
└── e2e_baselines/           # Tables 4 + 5: FCN/CRNN/CNNT, adapted from Ríos-Vila et al.
```
...
- **PyTorch environment** -- `fine_tuning/`, `zero_shot_eval/smt/`, and
  `e2e_baselines/` (evaluation and training). Each has its own
  ...
  (e.g. `e2e_baselines/requirements_cluster.txt` pins `torch<2.1.0` for older
...
| FCN/CRNN/CNNT checkpoints | Not redistributed by the original `e2e-pianoform-system-based` repo either -- train them yourself using `e2e_baselines/main.py` with the provided `config/GrandStaff/*.gin` / `config/Camera_GrandStaff/*.gin` configs, or obtain from the original repository's own release channel if/when available |
...
- `zero_shot_eval/zeus/README.md` documents a genuine methodological
  ...
  numbers.
- `e2e_baselines/README.md` documents a related but smaller and *not fully
