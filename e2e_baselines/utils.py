...
def make_vocabulary(YSequences, pathToSave, nameOfVoc):
    logger.info(f"Building vocabulary from {len(YSequences)} sequences...")
    vocabulary = set()
    for seq_idx, samples in enumerate(YSequences):
        logger.info(f"  Sequence {seq_idx}: {len(samples)} samples")
        if samples and len(samples) > 0:
            logger.info(f"    First sample: {len(samples[0])} tokens")
            if samples[0]:
                logger.info(f"      Sample tokens: {samples[0][:10]}")
        for element in samples:
                vocabulary.update(element)
    
    logger.info(f"  Total vocabulary size: {len(vocabulary)}")
    if vocabulary:
        logger.info(f"  Sample tokens: {list(vocabulary)[:20]}")
