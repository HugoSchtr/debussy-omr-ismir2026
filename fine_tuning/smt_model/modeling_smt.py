...
class SMTModelForCausalLM(PreTrainedModel):
    ...
    def _reinitialize_positional_encodings(self):
        """Reinitialize PE buffers that were corrupted by meta-device init in transformers >= 5.x.
        
        The PE buffers use persistent=False (not saved in state_dict) and are computed
        during __init__. With newer transformers/PyTorch, from_pretrained() may use meta
        device initialization, causing these buffers to contain NaN values.
        """
        ...
        needs_fix = (torch.isnan(self.pos2D.pe).any().item() or 
                     torch.isnan(self.decoder.position_encoding.pe).any().item())
        
        # Also reinitialize if maxlen changed (e.g. fine-tuning with longer sequences)
        pe_too_short = self.decoder.position_encoding.pe.size(0) < int(config.maxlen)
        
        if needs_fix or pe_too_short:
            self.pos2D = PositionalEncoding2D(
                dim=config.d_model, h_max=h_max, w_max=w_max
            ).to(device)
            self.decoder.position_encoding = PositionalEncoding1D(
                dim=config.d_model, len_max=int(config.maxlen)
            ).to(device)
            reason = "NaN fix" if needs_fix else f"PE buffer extended to maxlen={config.maxlen}"
            logger.info(f"Reinitialized positional encoding buffers ({reason})")
    ...
    def forward(self, encoder_input, decoder_input, labels=None):
        ...
        if labels is not None:
            # Cast logits to FP32 for numerical stability with mixed precision
            logits = output.logits.float().permute(0, 2, 1).contiguous()
