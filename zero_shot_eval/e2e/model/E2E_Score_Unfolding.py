...
class RecurrentScoreUnfolding(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x, _ = self.dec_lstm(x)
        x = self.out_dense(x)
        # Clamp outputs before log_softmax to prevent numerical instability
...
class PositionalEncoding1D(nn.Module):
    ...
    def forward(self, x, start=0):
        ...
        if isinstance(start, int):
            ...
            return x  # Fixed: was missing return value 
...
class E2EStave_CNNT(nn.Module):
    ...
    def __init__(self, in_channels, out_cats, pretrain_path=None, img_height=None, height_reduction=None, out_channels=None, max_len=None):
        super(E2EStave_CNNT, self).__init__()  # Fixed: was E2EStave_CRNN
        self.encoder = Encoder(in_channels=in_channels)
        ...
        self.decoder = StaveTransformerDecoder(img_height, height_reduction, out_channels, out_cats, max_len)  # Fixed: was StaveRNNDecoder
...
def get_cnntrf_model(maxwidth, maxheight, in_channels, out_size, maxlen=None):
    ...
    try:
        ...
        print(f"Warning: Could not generate model summary for CNNT (this is okay, will be generated later)")
...
def get_stave_cnnt(maxwidth, maxheight, in_channels, out_size, maxlen=None):
    # Note: StaveTransformerDecoder needs maxlen parameter
    # Calculate it if not provided
    if maxlen is None:
        maxlen = int(((maxheight // 16) * (maxwidth // 8)) * 1.1)
    
    # StaveTransformerDecoder parameters could be passed via gin config or calculated
    img_height = maxheight
    height_reduction = 16  # From encoder downsampling
    out_channels = 512  # From encoder output
    
    model = E2EStave_CNNT(in_channels=in_channels, out_cats=out_size, img_height=img_height, 
                          height_reduction=height_reduction, out_channels=out_channels, max_len=maxlen)
    try:
        summary(model, input_size=[(1,in_channels,maxheight,maxwidth)], dtypes=[torch.float])
    except:
        print(f"Warning: Could not generate model summary for Stave CNNT (this is okay, will be generated later)")
