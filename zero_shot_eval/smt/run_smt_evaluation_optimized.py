#!/usr/bin/env python3
"""
SMT Evaluation Script for SLURM Cluster
...
Pipeline:
1. Image → SMT model → **ekern prediction
...
def load_dataset(base_dir: str, test_mode: Optional[str] = None,
                 ...
                 dataset_subdir: str = "paired_system_level") -> List[Dict]:
    """Load dataset from paired_system_level directory."""
    base_path = Path(base_dir) / dataset_subdir
    ...
    dataset = []
    documents = sorted([d for d in base_path.iterdir() if d.is_dir()])
...
# =============================================================================
# VOCABULARY COMPARISON — MODEL vs DATASET (token-level)
# =============================================================================
...
def run_vocabulary_comparison(model, gt_data: Dict[str, Dict], output_dir: Path,
    ...
    if total_oov_instances == 0:
        logging.info("   (Expected: Zeus _emit() drops non-ALL_TOKENS before output, so LMX is always ⊆ ALL_TOKENS)")

    # --- MusicXML-level OOV (the REAL vocabulary gap) ---
    # The vocabulary analysis identifies MusicXML features that Zeus cannot
    # represent: dynamics, wedges, pedals, exotic tuplets, etc.  These are
    # silently dropped during linearization and never appear as LMX tokens.
    musicxml_oov_by_category = {}
    musicxml_oov_total = 0
    n_samples_musicxml_oov = 0
    per_sample_analysis = {}  # sample_id → vocab_analysis entry
    ...
    if vocab_info:
        ...
        # Build per-sample lookup from vocab analysis
    ...
    for sample_id, gt_entry in gt_data.items():
        ...
        # OOV count from MusicXML analysis (features that Zeus dropped)
        analysis_entry = per_sample_analysis.get(sample_id, {})
        n_musicxml_oov = analysis_entry.get('oov_count', 0)
        # Density: dropped features / (emitted tokens + dropped features)
...
# =============================================================================
# These tokens exist in the Zeus LMX vocabulary (ALL_TOKENS) and appear in GT
# linearizations, but the kern→MusicXML conversion never generates the
# corresponding MusicXML elements (slurs, articulations, grace notes, tremolo).
# Filtered metrics (fSER, fCER, fLER) remove these from both GT and predicted
# sequences before computing Levenshtein distance, giving a fairer comparison
# for kern-based models.
...
def evaluate_all(dataset: List[Dict], musicxml_files: Dict[str, str],
    ...
    for sample in tqdm(dataset, desc="Evaluation"):
        ...
        # Get GT tokens — skip only if GT itself is missing
        ...
        # Determine prediction tokens.
        # Failed conversion or failed linearization → empty prediction (max error).
        # This is the standard evaluation convention: the model produced unreadable
        # output, which is penalized as if it produced nothing.
        pred_status = 'success'
        if sample_id not in musicxml_files:
            pred_tokens = []
            pred_lmx = ""
            pred_status = 'no_musicxml'
        else:
            pred_lmx = linearize_musicxml(musicxml_files[sample_id], Linearizer, MxlFile, ET)
            if not pred_lmx:
                pred_tokens = []
                pred_lmx = ""
                pred_status = 'linearization_failed'
            else:
        ...
        gt_lmx_f = ' '.join(gt_tokens_f)
        pred_lmx_f = ' '.join(pred_tokens_f)
        ...
        results.append({
            ...
            # gt_lmx / pred_lmx stored for page-level LER aggregation below
            '_gt_lmx': gt_lmx,
            '_pred_lmx': pred_lmx,
            '_gt_lmx_f': gt_lmx_f,
            '_pred_lmx_f': pred_lmx_f,
            'conversion_method': conversion_methods.get(sample_id, 'unknown'),
        })

    # Compute global metrics
    # 'evaluated' = all samples with a GT (includes empty-prediction failures)
    evaluated_results = [r for r in results if 'SER' in r]
    ...
    # Folio-level LER: group systems by (document, folio), sort by sample_id to
    # preserve system order, then compute sequence Levenshtein on the ordered list
    # of LMX strings per folio and aggregate across folios.
    folio_groups: Dict[tuple, list] = defaultdict(list)
    for r in evaluated_results:
        folio_groups[(r['document'], r['folio'])].append(r)

    total_folio_edit = 0
    total_folio_gt_lines = 0
    folio_lers = []
    total_folio_edit_f = 0
    total_folio_gt_lines_f = 0
    folio_flers = []
    for (doc, folio), group in sorted(folio_groups.items()):
        group_sorted = sorted(group, key=lambda x: x['sample_id'])
        gt_lines = [x['_gt_lmx'] for x in group_sorted]
        pred_lines = [x['_pred_lmx'] for x in group_sorted]
        edit = levenshtein_tokens(gt_lines, pred_lines)
        n_lines = len(gt_lines)
        total_folio_edit += edit
        total_folio_gt_lines += n_lines
        folio_lers.append(edit / n_lines * 100 if n_lines > 0 else 0)
        # Filtered LER
        gt_lines_f = [x['_gt_lmx_f'] for x in group_sorted]
        pred_lines_f = [x['_pred_lmx_f'] for x in group_sorted]
        edit_f = levenshtein_tokens(gt_lines_f, pred_lines_f)
        total_folio_edit_f += edit_f
        total_folio_gt_lines_f += n_lines
        folio_flers.append(edit_f / n_lines * 100 if n_lines > 0 else 0)

    n_folios_evaluated = len(folio_lers)
    global_ler = total_folio_edit / total_folio_gt_lines * 100 if total_folio_gt_lines > 0 else 0
    global_fler = total_folio_edit_f / total_folio_gt_lines_f * 100 if total_folio_gt_lines_f > 0 else 0

    sample_sers = [r['SER'] for r in evaluated_results]
    ...
    sample_cers = [r['CER'] for r in evaluated_results]
    ...
    mean_ler = float(np.mean(folio_lers)) if folio_lers else 0
    std_ler = float(np.std(folio_lers, ddof=1)) if len(folio_lers) > 1 else 0
    median_ler = float(np.median(folio_lers)) if folio_lers else 0

    sample_fsers = [r['fSER'] for r in evaluated_results]
    ...
    sample_fcers = [r['fCER'] for r in evaluated_results]
    ...
    mean_fler = float(np.mean(folio_flers)) if folio_flers else 0
    std_fler = float(np.std(folio_flers, ddof=1)) if len(folio_flers) > 1 else 0
    median_fler = float(np.median(folio_flers)) if folio_flers else 0

    pct_unfair_gt = (total_unfair_gt / total_gt_tokens * 100) if total_gt_tokens > 0 else 0

    n_failed = n_evaluated - n_success
    logging.info(f"{'=' * 60}")
    ...
    logging.info(f"       strings grouped by folio. Each system's full LMX is one element.")
    logging.info(f"       Aggregated per-folio (document page), then globally.")
    logging.info(f"")
    ...
    logging.info(f"fLER = Filtered LER: same as LER on filtered LMX strings.")
    logging.info(f"")
    logging.info(f"{'=' * 60}")
    logging.info(f"EVALUATION RESULTS")
    logging.info(f"{'=' * 60}")
    logging.info(f"Evaluated: {n_evaluated}/{len(dataset)} samples ({n_success} success, {n_failed} empty-prediction)")
    ...
    logging.info(f"   Full dataset:           {global_ser:.2f}% (n={n_evaluated})")
    logging.info(f"")
    logging.info(f"PER-SAMPLE SER (n={n_evaluated}, incl. empty-prediction failures):")
    ...
    logging.info(f"   Full dataset:           {global_cer:.2f}% (n={n_evaluated})")
    logging.info(f"")
    logging.info(f"PER-SAMPLE CER (n={n_evaluated}):")
    logging.info(f"   Mean +/- Std:           {mean_cer:.2f}% +/- {std_cer:.2f}%")
    logging.info(f"   Median:                 {median_cer:.2f}%")
    logging.info(f"")
    logging.info(f"GLOBAL LER (folio-level: sequence Levenshtein over ordered systems per folio):")
    logging.info(f"   Full dataset:           {global_ler:.2f}% ({n_folios_evaluated} folios, {total_folio_gt_lines} systems)")
    logging.info(f"")
    logging.info(f"PER-FOLIO LER (n={n_folios_evaluated} folios):")
    ...
    logging.info(f"   Full dataset:           {global_fser:.2f}% (n={n_evaluated})")
    logging.info(f"")
    ...
    logging.info(f"   Full dataset:           {global_fcer:.2f}% (n={n_evaluated})")
    logging.info(f"")
    ...
    logging.info(f"   Full dataset:           {global_fler:.2f}% ({n_folios_evaluated} folios)")
    logging.info(f"")
    logging.info(f"PER-FOLIO fLER:")
    ...
    summary = {
        ...
        'model': 'PRAIG/smt-grandstaff',
        'total_dataset': len(dataset),
        'evaluated': n_evaluated,
        'success': n_success,
        'empty_prediction_failures': n_evaluated - n_success,
        'no_gt': len(dataset) - n_evaluated,
        ...
        'total_folio_edit': int(total_folio_edit),
        'total_folio_gt_lines': int(total_folio_gt_lines),
        'n_folios_evaluated': n_folios_evaluated,
        ...
        'failed_samples': [
            ...
            for r in evaluated_results if r.get('status') != 'success'
        ],
    }
...
def run_tedn_evaluation(dataset: List[Dict], musicxml_files: Dict[str, str],
    ...
    """
    Run TEDn (Tree Edit Distance, normalized) evaluation for both 'full' and 'lmx' flavors.
    Each sample is processed once and both flavors are computed in the same worker call.
    ...
    # Build (sample_id, pred_lmx, gt_path) tuples.
    # Samples where conversion/linearization failed use empty LMX ("") so that
    # TEDn_lmx_xml delinearizes to an empty <part> and scores maximum cost.
    tasks = []
    for sample in dataset:
        sid = sample['sample_id']
        gt_path = sample['gt_musicxml_path']

        if sid not in musicxml_files:
            tasks.append((sid, "", gt_path))
            continue

        pred_lmx = linearize_musicxml(musicxml_files[sid], Linearizer, MxlFile, ET)
        tasks.append((sid, pred_lmx or "", gt_path))
    ...
    global_tedn_full = (total_edit_full / total_gold_full * 100) if total_gold_full > 0 else 0
    global_tedn_lmx = (total_edit_lmx / total_gold_lmx * 100) if total_gold_lmx > 0 else 0

    sample_tedns_full = [r['TEDn_full'] for r in tedn_results]
    sample_tedns_lmx = [r['TEDn_lmx'] for r in tedn_results]
    mean_tedn_full = float(np.mean(sample_tedns_full)) if sample_tedns_full else 0
    mean_tedn_lmx = float(np.mean(sample_tedns_lmx)) if sample_tedns_lmx else 0
    median_tedn_full = float(np.median(sample_tedns_full)) if sample_tedns_full else 0
    median_tedn_lmx = float(np.median(sample_tedns_lmx)) if sample_tedns_lmx else 0
    ...
    logging.info(f"   Global TEDn-full: {global_tedn_full:.2f}%")
    logging.info(f"   Mean TEDn-full:   {mean_tedn_full:.2f}%")
    logging.info(f"   Median TEDn-full: {median_tedn_full:.2f}%")
    logging.info(f"")
    logging.info(f"TEDn-lmx (LMX-covered concepts only):")
    logging.info(f"   Global TEDn-lmx:  {global_tedn_lmx:.2f}%")
    logging.info(f"   Mean TEDn-lmx:    {mean_tedn_lmx:.2f}%")
    logging.info(f"   Median TEDn-lmx:  {median_tedn_lmx:.2f}%")
    logging.info(f"{'=' * 60}")

    # Save per-sample TEDn results
    ...
    tedn_summary = {
        ...
        'failed': failed,
        'full': {
            'global_TEDn': round(global_tedn_full, 4),
            'mean_TEDn': round(mean_tedn_full, 4),
            'median_TEDn': round(median_tedn_full, 4),
            'total_edit_cost': int(total_edit_full),
            'total_gold_cost': int(total_gold_full),
        },
        'lmx': {
            'global_TEDn': round(global_tedn_lmx, 4),
            'mean_TEDn': round(mean_tedn_lmx, 4),
            'median_TEDn': round(median_tedn_lmx, 4),
...
def save_report(summary: Dict, conversion_stats: Dict, vocab_info: Dict,
    ...
    with open(report_file, 'w') as f:
        ...
        f.write("SER (Symbol Error Rate):\n")
        f.write("  Token-level Levenshtein edit distance between GT and predicted LMX\n")
        f.write("  token sequences, normalized by GT token count.\n\n")
        f.write("CER (Character Error Rate):\n")
        f.write("  Character-level Levenshtein on concatenated token strings (no spaces),\n")
        f.write("  normalized by GT character count.\n\n")
        f.write("LER (Line Error Rate):\n")
        f.write("  Sequence-level Levenshtein over system LMX strings grouped by folio.\n")
        f.write("  Each system's full LMX string is one element in the sequence.\n")
        f.write("  Aggregated per-folio (document page), then globally.\n\n")
        f.write("DATASET\n")
        f.write("-" * 70 + "\n")
        f.write(f"Total samples:              {summary['total_dataset']}\n")
        f.write(f"Evaluated (has GT):         {summary['evaluated']}\n")
        f.write(f"  Fully parsed:             {summary['success']}\n")
        f.write(f"  Empty-prediction (failed):{summary['empty_prediction_failures']}\n")
        f.write(f"  No GT linearization:      {summary['no_gt']}\n\n")
        ...
        f.write("LER (Folio-level: sequence Levenshtein over ordered systems per folio)\n")
        f.write("-" * 70 + "\n")
        f.write(f"Global LER:                  {summary['global_LER']:.2f}%\n")
        f.write(f"Folios evaluated:            {summary['n_folios_evaluated']}\n")
        f.write(f"Total systems:               {summary['total_folio_gt_lines']}\n")
        f.write(f"Mean +/- Std (per folio):    {summary['mean_LER']:.2f}% +/- {summary['std_LER']:.2f}%\n")
        f.write(f"Median (per folio):          {summary['median_LER']:.2f}%\n\n")
...
def main():
    ...
    dataset = load_dataset(
        ...
        dataset_subdir=config.get('dataset_subdir', 'paired_system_level'),
    )
    ...
    # Vocabulary analysis (MusicXML-level OOV scan — sample-level)
    ...
    if config.get('enable_tedn', False):
        logger.info("Running TEDn evaluation (both full and lmx flavors — this may take a while)...")
